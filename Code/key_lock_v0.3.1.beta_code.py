...
class KeyLockFrame(wx.Frame):
    def __init__(self):
        super(KeyLockFrame, self).__init__(None, title="Key-Lock 版本:V0.3.1.beta 版权所有:XeroL©", size=(450, 200))
        panel = wx.Panel(self)

        self.welcome_label = wx.StaticText(panel, label="欢迎使用 Key-Lock 文件加解密工具！", pos=(120, 20), size=(300, 25))
        self.ad_label = wx.StaticText(panel, label="GitHub网站: https://github.com/WeiXuL/KeyLock.git", pos=(50, 120), size=(300, 25))
        self.choice_label = wx.StaticText(panel, label="请选择操作：", pos=(50, 50), size=(200, 25))
        self.gen_key_btn = wx.Button(panel, label="生成密钥", pos=(50, 75), size=(150, 30))
        self.enc_dec_btn = wx.Button(panel, label="加密/解密文件", pos=(250, 75), size=(150, 30))
